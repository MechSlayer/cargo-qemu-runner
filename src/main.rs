...
fn prepare_iso(
    ...
    if !std::path::Path::new(&format!("target/limine/{}_done", plain_iso_file)).exists() {
        ...
        std::fs::copy(
            ...
            &format!("{}/{}", iso_root, "limine-cd-efi.bin"),
        )
        .expect(&format!(
            "failed to copy file {}",
            &format!("{}/{}", iso_root, "limine-cd-efi.bin")
...
fn main() {
    ...
    let package = if let Some(package) = metadata.root_package() {
        ...
        let mut target_exe_path_buf = PathBuf::from(target_exe_path);
        target_exe_path_buf.set_extension("");

        let exe_name = target_exe_path_buf
            .file_name()
            .expect("no target executable name");

        let package_name = exe_name.to_str().unwrap();
        metadata
            .packages
            .iter()
            .find(|p| p.name.as_str() == package_name)
            .expect(&format!("no package found with name {}", package_name))
    };
